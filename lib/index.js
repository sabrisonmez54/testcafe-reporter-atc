...
const closeTestExecutionTicket = async (testExecutionKey) => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            console.error(`ATC Reporter: There was an error with the response for closing the execution ticket: ${testExecutionKey}`);
        }
    }
    catch (e) {
...
const sendXrayResultsToJira = async (xrayResults) => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.testExecIssue.key) {
                console.log(`ATC Reporter: Successfully sent xray results for ${currentTestMeta.jiraTestKey} to jira.`, data);
                await closeTestExecutionTicket(data.testExecIssue.key);
            }
        }
        else {
            console.error(`ATC Reporter: There was an issue with the response for sendXrayResultsToJira `);
        }
    }
    catch (e) {
        console.error(`ATC Reporter: There was an error sending the xray results to jira for ${currentTestMeta.jiraTestKey}.`, e);
    }
};
exports['default'] = () => {
    return {
