...
const closeTestExecutionTicket = async (testExecutionKey) => {
    ...
    try {
        console.log(`ATC Reporter: Closing execution ticket: ${testExecutionKey}...`);
    ...
    return {
        ...
        async reportTestDone(name, testRunInfo, meta) {
            ...
            if (!jiraMetaDataIsValid(currentTestMeta)) {
                console.warn(`ATC Reporter: JiraMetaData missing or undefined. ExecutionResult will not be collected.`, name);
                return;
            }
            let executionResult = {
                info: {
        ...
        async reportTaskDone(endTime, passed, warnings, result) {
            if (executionResults.length > 0) {
                ...
                if (!jiraAuthValid()) {
                    console.warn(`ATC Reporter: Can't upload test results. Jira authentication env variables are invalid. Make sure you define process.env.JIRA_USERNAME and process.env.JIRA_PASSWORD correctly.`, `Used jira auth: Basic ${base64data}`);
                }
                else {
                ...
                console.log('ATC Reporter: There are no execution results to upload to jira.');
            }
        },
    };
};
